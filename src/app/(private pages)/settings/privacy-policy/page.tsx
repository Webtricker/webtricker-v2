...
import { useGetPrivacyPolicyPageDataQuery, useUpdatePrivacyPolicyPageDataMutation } from "@/redux/features/pageData/pageData";
...
export default function PrivacyPolicyPage() {
    ...
    const { data, isLoading: isPageLoading } = useGetPrivacyPolicyPageDataQuery({})
    ...
    const [updatePrivacyPage, { isLoading }] = useUpdatePrivacyPolicyPageDataMutation();
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await updatePrivacyPage({ id: data?.data?._id || '', data: pageData }).unwrap();
            if (res.success) {
    ...
    return (
        <>
            <div className=" w-full lg:fixed z-50 top-0 py-3 px-4 md:px-5 lg:px-10 left-0 flex items-center justify-between lg:bg-slate-100">
                <PageTitle key="UPDATE_CONTACT" title="Update Privacy Page" />
