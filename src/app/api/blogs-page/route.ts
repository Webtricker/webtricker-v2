...
import BlogPage from "@/models/BlogsPage";
...
export const PUT = async (req: NextRequest) => {
    try {
        ...
        const portfolioPageData = await BlogPage.findByIdAndUpdate(id, data);
        ...
        return NextResponse.json(
            { success: true, message: "Data updated", data: portfolioPageData },
            { status: 200 }
        );
    } catch (error: any) {
        console.error(error);
