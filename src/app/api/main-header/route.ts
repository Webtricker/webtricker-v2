...
export const PUT = async (req: NextRequest) => {
    try {
        ...
        const topHeaderData = await MainHeader.findByIdAndUpdate(id, data);
...
export const GET = async () => {
    try {
        ...
        const data = await MainHeader.findOne().lean();
        ...
        console.error('Error fetching main header data.', error);
        return NextResponse.json(
            { success: false, message: 'Internal Server Error' },
            { status: 500 }
        );
